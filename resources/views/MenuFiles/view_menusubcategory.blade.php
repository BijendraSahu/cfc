...
@section('title','List of Menu SubCategory')
...
@section('content')
        ...
        <div class="container-fluid">
            <table id="dataTable" class="display compact" cellspacing="0" width="100%">
                ...
                @if(count($MenuSubCategory)>0)
                    @foreach($MenuSubCategory as $MenuSubCateg)
                        <tr>
                            <td class="hidden">{{$MenuSubCateg->McatID}}</td>
                            <td id="{{$MenuSubCateg->McatID}}">
                                <a href="#" id="{{$MenuSubCateg->McatID}}" class="btn btn-sm btn-default edit-unit_"
                                   title="Edit unit">
                                    <span class="fa fa-pencil"></span></a>
                                {{--@if($_SESSION['user_master']->id != $user_master->id)--}}

                                <button type="button" id="{{ $MenuSubCateg->McatID }}"
                                        class="btn btn-sm btn-danger btnDelete" title="Inactivate"><span
                                            class="fa fa-trash-o" aria-hidden="true"></span></button>
                                {{--@endif--}}
                            </td>
                            <td>{{$MenuSubCateg->category->CategoryName}}</td>
                            <td>{{$MenuSubCateg->CategoryName}}</td>
                            <td>{{$MenuSubCateg->Discriptions}}</td>
                        </tr>
                    @endforeach
                @endif
                </tbody>
    ...
    <script>
        $('.btnDelete').click(function () {
            ...
            $('#modalBtn').html('<a class="btn btn-sm btn-danger" href="{{ url('subcategory') }}/' + id +
...
//            $('#modelBtn').visible(disabled);
        ...
        $(".edit-unit_").click(function () {
            ...
            var id = $(this).attr('id');
            var editurl = '{{ url('/') }}' + "/subcategory/" + id + "/edit";
            $.ajax({
                type: "GET",
                contentType: "application/json; charset=utf-8",
                url: editurl,
