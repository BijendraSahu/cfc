...
@section('content')
    ...
    <a href="#" class="btn btn-sm bg-danger btnSet btn-primary add-user pull-right">
        <span class="fa fa-plus"></span>&nbsp;Create New Emp Type</a>
    <h3 class="heading">List of Type</h3>
    <hr/>
    ...
    <div class="row fa-border">
        <div class="container-fluid">
            <table id="dataTable" class="display compact" cellspacing="0" width="100%">
                ...
                <tbody>
                @if(count($EmplyoeeType)>0)
                    @foreach($EmplyoeeType as $EmplyoeeType)
                        <tr>
                            <td class="hidden">{{$EmplyoeeType->EPTID}}</td>
                            <td id="{{$EmplyoeeType->EPTID}}">
                                <a href="#" id="{{$EmplyoeeType->EPTID}}" class="btn btn-sm btn-default edit-unit_"
                                   title="Edit unit">
                                    <span class="fa fa-pencil"></span></a>
                                {{--@if($_SESSION['user_master']->id != $user_master->id)--}}

                                <button type="button" id="{{ $EmplyoeeType->EPTID }}"
                                        class="btn btn-sm btn-danger btnDelete" title="Inactivate"><span
                                            class="fa fa-trash-o" aria-hidden="true"></span></button>
                                {{--@endif--}}
                            </td>
                            <td>{{$EmplyoeeType->Emp_Type}}</td>
    ...
    <script>
        $('.btnDelete').click(function () {
            ...
            $('#modalBtn').html('<a class="btn btn-sm btn-danger" href="{{ url('EMPTYPE') }}/' + id +
...
//            $('#modelBtn').visible(disabled);
        ...
        $(".edit-unit_").click(function () {
            ...
            var id = $(this).attr('id');
            var editurl = '{{ url('/') }}' + "/EMPTYPE/" + id + "/edit";
            $.ajax({
                type: "GET",
                contentType: "application/json; charset=utf-8",
                url: editurl,
